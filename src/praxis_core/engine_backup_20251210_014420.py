...
def attach_reliability(
    ...
    for row in pra_rows:
        ...
        reliability_1y = math.exp(-lambda_eff)
...
def get_pra_key(row: dict):
    """
    Return (domain, risk, failure_class) tuple for a basic event.

    If 'pra_key' exists and is a 3-element list/tuple, use that.
    Otherwise, fall back to the row's own domain/risk/failure_class fields.
    """
    if "pra_key" in row and row["pra_key"]:
        key = row["pra_key"]
        # Allow pra_key to be either a list/tuple or a single string
        if isinstance(key, (list, tuple)) and len(key) == 3:
            return key[0], key[1], key[2]
        if isinstance(key, str):
            # You can change this split logic later if you want a custom format
            parts = key.split(" | ")
            if len(parts) == 3:
                return parts[0], parts[1], parts[2]

    # Fallback: derive directly from the row
    return row.get("domain", "UNKNOWN_DOMAIN"), \
