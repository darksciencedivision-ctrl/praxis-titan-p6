...
def _iter_mode_config(twin_config: Any) -> List[Tuple[str, int]]:
    ...
    or some other shape (list, None, etc.). In those cases we gracefully
    fall back to defaults.
    """
    default_count = 6

    # If the JSON was a list or something weird, just ignore it and use defaults
    if not isinstance(twin_config, dict):
        twin_config = {}

    modes_cfg = twin_config.get("modes", {}) or {}

    out: List[Tuple[str, int]] = []
    for mode in ("optimistic", "pessimistic", "chaotic"):
        cfg = modes_cfg.get(mode, {}) or {}
        try:
