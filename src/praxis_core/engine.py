...
# Optional modules (may or may not exist in your tree)
try:
...
def write_json(path: Path, data: Dict[str, Any]) -> None:
    """Write JSON with indentation, creating parent folders if needed."""
...
def build_basic_probs_from_numeric(numeric_results: Any) -> Dict[str, float]:
    """
    Normalize whatever numeric.compute_numeric_risks(...) returns into:

        { risk_id: probability }

    Supports both:
      - dict with "basic_probs" or "probs"
      - list[dict] of rows with id + p_base/likelihood/p
    """
    # Case 1: already a dict
    if isinstance(numeric_results, dict):
        # Direct mapping if present
        probs = (
            numeric_results.get("basic_probs")
            or numeric_results.get("probs")
            or {}
        )
        if isinstance(probs, dict) and probs:
            return probs

        # Or maybe nested under "risks"
        out: Dict[str, float] = {}
        for row in numeric_results.get("risks", []):
...
def run_baseline_pipeline(
    ...
    # 2. Turn numeric_results into {id: p} regardless of its shape
    basic_probs: Dict[str, float] = build_basic_probs_from_numeric(numeric_results)
    ...
    try:
        ...
        # Fallback for older bayes.py signature
    ...
    # 6. Fault-tree analytic
    ft_structure = scenario.get("fault_tree", {})
    ft_analytic: Dict[str, Any] = {}
    ...
    # 7. Fault-tree Monte Carlo (if available)
    ft_mc: Dict[str, Any] = {}
